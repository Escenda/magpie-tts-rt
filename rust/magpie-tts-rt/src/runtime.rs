...
impl Request {
    ...
    pub fn acquire_audio(&mut self) -> Result<Option<AudioLease<'_>>> {
        ...
        let validated_position = self.validate_audio_lease(&raw).and_then(|()| {
            ...
            let next_sample_index = raw.first_sample_index.checked_add(raw.sample_count).ok_or(
            ...
            Ok((next_sequence, next_sample_index))
        });
        let next_audio_position = match validated_position {
    ...
    fn validate_audio_lease(&mut self, raw: &sys::mtt_audio_lease_v1_t) -> Result<()> {
        ...
        if raw.samples.addr() % align_of::<f32>() != 0 {
            return Err(invalid_native(
                operation,
                "samples",
        ...
        if raw.sample_count == 0 || raw.sample_count % CODEC_FRAME_SAMPLES != 0 {
            return Err(invalid_native(
                operation,
                "sample_count",
                "must contain one or more complete 1,024-sample codec frames",
            ));
        }
        ...
        if raw.flags & ALIGNMENT_VALID == 0 && raw.committed_text_tokens != 0 {
            return Err(invalid_native(
                operation,
                "committed_text_tokens",
