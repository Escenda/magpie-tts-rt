...
class StreamingState:
    ...
    local_ar_output_codes: Optional[torch.Tensor] = None
    local_ar_random_state: Optional[LocalARRandomState] = None
...
class EasyMagpieTTSInferenceModel(ModelPT):
    ...
    def _apply(self, fn, recurse=True):
        """Invalidate captured local-AR graphs before moving model storage."""
    ...
    def _sample_audio_codes(
        ...
        use_cfg: bool,
        cfg_scale: float,
        output_codes: Optional[torch.Tensor],
        ...
        if use_local_transformer_for_inference:
            if self.local_transformer_type == LocalTransformerType.AR:
                if output_codes is None:
                    raise ValueError("Local-AR streaming inference requires a caller-owned output buffer")
    ...
    def streaming_init(
        ...
        with grad_ctx():
            ...
            local_ar_output_codes = None
            local_ar_random_state = None
            if use_local_transformer:
                local_ar_output_codes = torch.empty(
                    (
                        batch_size,
                        self.num_audio_codebooks,
                        self.frame_stacking_factor,
            ...
            state = StreamingState(
                ...
                local_ar_output_codes=local_ar_output_codes,
                local_ar_random_state=local_ar_random_state,
    ...
    def _process_predictions(
        ...
        if needs_audio.any():
            ...
            if state.config.use_local_transformer:
                state.all_predictions.append(audio_codes_unstacked.clone())
            else:
    ...
    def _predict_audio_codes(self, state: StreamingState) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        audio_codes_next, all_codes_next_argmax = self._sample_audio_codes(
            ...
            output_codes=state.local_ar_output_codes,
            random_state=state.local_ar_random_state,
        )
