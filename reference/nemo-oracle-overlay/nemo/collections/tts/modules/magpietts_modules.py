...
@dataclass(frozen=True)
class _LocalARParameterSignature:
    name: str
    data_ptr: int
    version: int
    shape: tuple[int, ...]
...
class _LocalARCudaGraphRuntime:
    """Fixed-shape CUDA graph for one local-AR sampling configuration.

    A runtime is cached by ``LocalTransformerHelper`` and owns reusable
    input/status buffers plus incremental KV storage. Every replay overwrites
    all 16 positions. A lock and CUDA event serialize callers across streams,
    so state never crosses requests semantically even though storage is reused.
    """
    ...
    def __init__(
        ...
        key: _LocalARCudaGraphKey,
        *,
        capture_graph: bool = True,
    ):
        if not torch.cuda.is_available():
            raise RuntimeError("Local-AR CUDA graph runtime requires CUDA")
        if key.input_batch_size < 1:
            raise ValueError(f"input_batch_size must be positive, got {key.input_batch_size}")
        if key.use_cfg and key.input_batch_size % 2 != 0:
            raise ValueError(
                f"CFG local-AR input batch must contain equal conditional/unconditional halves, "
                f"got {key.input_batch_size}"
            )

        self.device = torch.device("cuda", key.device_index)
        self.helper = helper
        self.key = key
        self.actual_batch_size = key.input_batch_size // 2 if key.use_cfg else key.input_batch_size
        self._lock = Lock()
        ...
        self.static_dec_output = torch.zeros(
            (key.input_batch_size, key.input_dim),
            device=self.device,
            dtype=key.dtype,
        )
        self.static_unfinished_mask = torch.zeros(
            self.actual_batch_size,
            device=self.device,
            dtype=torch.bool,
        )
        self.static_finished_mask = torch.zeros_like(self.static_unfinished_mask)
        self.static_forbid_eos_mask = torch.zeros_like(self.static_unfinished_mask)
        self.static_invalid_rows = torch.zeros(
            self.actual_batch_size,
            device=self.device,
            dtype=torch.int32,
        )
        self.static_random_state = LocalARRandomState.create(
            actual_batch_size=self.actual_batch_size,
            device=self.device,
            seed=0,
        )

        sequence_length = helper.num_audio_codebooks * helper.frame_stacking_factor
        vocab_size = helper.codebook_size + len(SpecialAudioToken)
        if len(helper.audio_embeddings) != sequence_length:
            raise ValueError(
                f"Local-AR requires one embedding per sequence position: "
                f"expected {sequence_length}, got {len(helper.audio_embeddings)}"
            )
        if len(helper.local_transformer_out_projections) != sequence_length:
            raise ValueError(
                f"Local-AR requires one output projection per sequence position: "
                f"expected {sequence_length}, got {len(helper.local_transformer_out_projections)}"
            )
        embedding_dim = helper.audio_embeddings[0].embedding_dim
        for position, (embedding, projection) in enumerate(
            zip(helper.audio_embeddings, helper.local_transformer_out_projections)
        ):
            if embedding.num_embeddings != vocab_size or embedding.embedding_dim != embedding_dim:
                raise ValueError(
                    f"Embedding {position} must have shape ({vocab_size}, {embedding_dim}), "
                    f"got ({embedding.num_embeddings}, {embedding.embedding_dim})"
                )
            if projection.out_features != vocab_size:
                raise ValueError(
                    f"Output projection {position} must produce {vocab_size} logits, " f"got {projection.out_features}"
                )
            if embedding.weight.dtype != key.dtype:
                raise ValueError(
                    f"Embedding {position} dtype {embedding.weight.dtype} does not match local-AR dtype {key.dtype}"
                )
        ...
        self.next_embedding = torch.empty(
            (key.input_batch_size, embedding_dim),
            device=self.device,
            dtype=key.dtype,
        )
        self.step_mask = torch.ones(
            (key.input_batch_size, 1),
            device=self.device,
            dtype=torch.bool,
        )
        self.incremental_state = helper.local_transformer.create_incremental_state(
            batch_size=key.input_batch_size,
            max_length=helper.num_audio_codebooks * helper.frame_stacking_factor + 1,
            device=self.device,
            dtype=key.dtype,
        )
        self.fused_sampler = FusedLocalARSampler(
            FusedLocalARSamplingConfig(
                actual_batch_size=self.actual_batch_size,
                vocab_size=vocab_size,
                codebook_size=helper.codebook_size,
                audio_eos_id=helper.audio_eos_id,
                top_k=key.topk,
                temperature=key.temperature,
                cfg_scale=key.cfg_scale,
                use_cfg=key.use_cfg,
            ),
            self.device,
        )

        self.graph = torch.cuda.CUDAGraph() if capture_graph else None
    ...
    def _warmup_and_capture_graph(self) -> None:
        """Compile and capture the fixed graph before request-time replay.

        This is the only runtime path that performs a device-wide
        synchronization. ``run`` only orders the caller and graph streams with
        events.
        """

        if self.graph is None:
            raise RuntimeError("A direct-capture local-AR body does not own an inner CUDA graph")
    ...
    def _runtime_named_parameters(self) -> Iterator[tuple[str, torch.nn.Parameter]]:
        module_groups = (
            ("local_transformer", self.helper.local_transformer),
            ("audio_embeddings", self.helper.audio_embeddings),
            ("audio_in_projection", self.helper.audio_in_projection),
            ("local_transformer_in_projection", self.helper.local_transformer_in_projection),
            ("local_transformer_audio_out_projection", self.helper.local_transformer_audio_out_projection),
            ("local_transformer_out_projections", self.helper.local_transformer_out_projections),
        )
        for group_name, module in module_groups:
            for parameter_name, parameter in module.named_parameters():
                yield f"{group_name}.{parameter_name}", parameter

    def _read_parameter_signature(self) -> tuple[_LocalARParameterSignature, ...]:
        return tuple(
            _LocalARParameterSignature(
                name=name,
            ...
            for name, parameter in self._runtime_named_parameters()
        )

    def _record_parameter_invariants(self) -> tuple[_LocalARParameterSignature, ...]:
        signature = self._read_parameter_signature()
        wrong_device = tuple(item.name for item in signature if item.device != self.device)
        if wrong_device:
            raise RuntimeError(
                f"Local-AR graph parameters must all be on {self.device}; "
                f"found different devices for {list(wrong_device)}"
            )
        return signature

    def validate_parameter_invariants(self) -> None:
        """Validate a cached graph only when warmup is explicitly requested."""

        current_signature = self._record_parameter_invariants()
        if current_signature != self.parameter_signature:
            raise RuntimeError(
                "Local-AR parameters changed or moved after CUDA Graph capture. "
                "Create a new inference model and explicitly warm it before serving requests"
            )

    def _execute_fixed_shape(self) -> None:
        self.static_invalid_rows.zero_()
        x_step = self.helper.local_transformer_in_projection(self.static_dec_output.unsqueeze(1))
        sequence_length = self.helper.num_audio_codebooks * self.helper.frame_stacking_factor

        for codebook_index in range(sequence_length):
            transformer_output = self.helper.local_transformer.forward_incremental(
                x_step=x_step,
                x_mask=self.step_mask,
                state=self.incremental_state,
                position_offset=codebook_index,
            ).output
            projection_input = self.helper.local_transformer_audio_out_projection(transformer_output[:, 0, :])
            logits = self.helper.local_transformer_out_projections[codebook_index](projection_input)
            token_output = self.output_tokens[:, codebook_index]
            if codebook_index + 1 < sequence_length:
                self.fused_sampler.sample_into(
                    logits,
                    self.static_unfinished_mask,
                    self.static_finished_mask,
                    self.static_forbid_eos_mask,
                    token_output,
                    self.helper.audio_embeddings[codebook_index].weight,
                    self.next_embedding,
                    random_state=self.static_random_state,
                    invalid_rows=self.static_invalid_rows,
                )
                token_embedding = self.helper.audio_in_projection(self.next_embedding.unsqueeze(1))
                x_step = self.helper.local_transformer_in_projection(token_embedding)
            else:
                self.fused_sampler.sample_into(
                    logits,
                    self.static_unfinished_mask,
    ...
    def _prepare_status_masks(
        self,
        unfinished_items: dict[int, bool],
        finished_items: dict[int, bool],
        forbid_audio_eos: bool,
    ) -> None:
        unfinished_indices = tuple(index for index, active in unfinished_items.items() if active)
        finished_indices = tuple(index for index, active in finished_items.items() if active)
        overlap = set(unfinished_indices).intersection(finished_indices)
        if overlap:
            raise ValueError(f"Local-AR items cannot be both unfinished and finished: {sorted(overlap)}")
        if forbid_audio_eos and finished_indices:
            raise ValueError(
                "Local-AR cannot force EOS for finished items while globally forbidding audio EOS: "
                f"{list(finished_indices)}"
            )

        for index in unfinished_indices + finished_indices:
            if index < 0 or index >= self.actual_batch_size:
                raise IndexError(f"Local-AR item index {index} is outside actual batch size {self.actual_batch_size}")

        self.static_unfinished_mask.zero_()
        self.static_finished_mask.zero_()
        self.static_forbid_eos_mask.fill_(forbid_audio_eos)
        for index in unfinished_indices:
            self.static_unfinished_mask[index : index + 1].fill_(True)
        for index in finished_indices:
            self.static_finished_mask[index : index + 1].fill_(True)
    ...
    def run(
        ...
        forbid_audio_eos: bool,
        output_codes: Tensor,
        random_state: LocalARRandomState | None,
    ) -> Tensor:
        if self.graph is None:
            raise RuntimeError(
                "The direct-capture local-AR body cannot replay independently; "
                "execute it only while building its owning outer CUDA graph"
            )
        if dec_output.shape != self.static_dec_output.shape:
            raise ValueError(
                f"Captured local-AR input shape is {tuple(self.static_dec_output.shape)}, "
                f"received {tuple(dec_output.shape)}"
            )
        if dec_output.device != self.device or dec_output.dtype != self.key.dtype:
            raise ValueError(
                f"Captured local-AR input is {self.device}/{self.key.dtype}, "
                f"received {dec_output.device}/{dec_output.dtype}"
            )
        expected_output_shape = (
            self.actual_batch_size,
            self.helper.num_audio_codebooks,
            self.helper.frame_stacking_factor,
        )
        if output_codes.shape != expected_output_shape:
            raise ValueError(f"output_codes must have shape {expected_output_shape}, got {tuple(output_codes.shape)}")
        if output_codes.device != self.device or output_codes.dtype != torch.int64:
            raise ValueError(f"output_codes must be an int64 tensor on {self.device}")
        stochastic = self.key.temperature > 0.0 and self.key.topk > 1
        if stochastic:
            if random_state is None:
                raise ValueError("Stochastic local-AR inference requires a session-owned LocalARRandomState")
            random_state.validate(actual_batch_size=self.actual_batch_size, device=self.device)
        ...
        with self._lock:
            ...
            with torch.cuda.stream(self.execution_stream):
                self._prepare_status_masks(unfinished_items, finished_items, forbid_audio_eos)
                self.static_dec_output.copy_(dec_output)
                if stochastic:
                    self.static_random_state.seed.copy_(random_state.seed)
                    self.static_random_state.counters.copy_(random_state.counters)
                self.graph.replay()
                if stochastic:
                    random_state.counters.copy_(self.static_random_state.counters)
                torch.ops.aten._assert_async.msg(
                    (self.static_invalid_rows == 0).all(),
                    "Local-AR graph received NaN, infinity, or no valid candidate",
                )
                output_codes.copy_(
                    self.output_tokens.reshape(
                        self.actual_batch_size,
                        self.helper.frame_stacking_factor,
                        self.helper.num_audio_codebooks,
                    ).permute(0, 2, 1)
            ...
            output_codes.record_stream(caller_stream)

        return output_codes

    def synchronize_before_release(self) -> None:
        """Wait until this runtime no longer has pending CUDA work."""

        with self._lock:
            self.execution_stream.synchronize()


class LocalARDirectCaptureRuntime(_LocalARCudaGraphRuntime):
    """Dedicated local-AR body captured directly by one outer CUDA graph.

    Unlike the normal local-AR runtime, this object never creates or replays an
    inner graph. Its buffers and incremental state belong exclusively to the
    outer runtime, which must hold its lease until decoder continuation ends.
    """

    def __init__(self, helper: LocalTransformerHelper, key: _LocalARCudaGraphKey):
        super().__init__(helper, key, capture_graph=False)

    def copy_random_state_from(self, random_state: LocalARRandomState | None) -> None:
        """Initialize graph-owned stochastic state before an outer replay."""

        stochastic = self.key.temperature > 0.0 and self.key.topk > 1
        if stochastic:
            if random_state is None:
                raise ValueError("Stochastic local-AR outer capture requires a session-owned random state")
            random_state.validate(actual_batch_size=self.actual_batch_size, device=self.device)
            self.static_random_state.seed.copy_(random_state.seed)
            self.static_random_state.counters.copy_(random_state.counters)
        elif random_state is not None:
    ...
    def copy_random_state_to(self, random_state: LocalARRandomState | None) -> None:
        """Commit graph-owned stochastic counters after an outer replay."""
    ...
    def execute_capture_step(
        ...
        *,
        ...
        """Record one fixed local-AR body into the caller's outer graph."""

        if dec_output.shape != self.static_dec_output.shape:
            raise ValueError(
                f"Direct-capture local-AR input shape is {tuple(self.static_dec_output.shape)}, "
                f"received {tuple(dec_output.shape)}"
            )
        if dec_output.device != self.device or dec_output.dtype != self.key.dtype:
            raise ValueError(
                f"Direct-capture local-AR input is {self.device}/{self.key.dtype}, "
                f"received {dec_output.device}/{dec_output.dtype}"
            )
        expected_output_shape = (
            self.actual_batch_size,
        ...
        if output_codes.device != self.device or output_codes.dtype != torch.int64:
            raise ValueError(f"output_codes must be an int64 tensor on {self.device}")
        if invalid_rows.shape != self.static_invalid_rows.shape:
            raise ValueError(
                f"invalid_rows must have shape {tuple(self.static_invalid_rows.shape)}, "
                f"got {tuple(invalid_rows.shape)}"
            )
        if invalid_rows.device != self.device or invalid_rows.dtype != torch.int32:
            raise ValueError(f"invalid_rows must be an int32 tensor on {self.device}")

        self._prepare_status_masks({}, {}, forbid_audio_eos)
        self.static_dec_output.copy_(dec_output)
        self._execute_fixed_shape()
        invalid_rows.copy_(self.static_invalid_rows)
        output_codes.copy_(
            self.output_tokens.reshape(
                self.actual_batch_size,
                self.helper.frame_stacking_factor,
...
class LocalTransformerHelper:
    ...
    def __init__(
        ...
        self._ar_cuda_graph_runtimes: dict[_LocalARCudaGraphKey, _LocalARCudaGraphRuntime] = {}
        self._ar_cuda_graph_lifecycle = Condition(Lock())
        self._ar_cuda_graph_active_runs = 0
        self._ar_cuda_graph_invalidation_active = False
    ...
    def create_random_state(
        *,
        ...
        """Create stochastic state owned by one generation call."""

        return LocalARRandomState.create(
    ...
    def _require_autoregressive_inference_mode(self) -> None:
        if self.local_transformer.training:
            raise RuntimeError(
                "Local-AR CUDA Graph inference requires local_transformer.eval(); "
                "training mode cannot capture or replay a graph"
            )

    def _build_autoregressive_graph_key(
        self,
        *,
        device: torch.device,
        dtype: torch.dtype,
        ...
        use_cfg: bool,
        cfg_scale: float,
    ) -> _LocalARCudaGraphKey:
        if device.type != "cuda":
            raise ValueError(f"The local-AR inference runtime requires CUDA, received {device}")
        if input_batch_size < 1:
            raise ValueError(f"input_batch_size must be positive, got {input_batch_size}")
        if input_dim < 1:
            raise ValueError(f"input_dim must be positive, got {input_dim}")
        if use_cfg and input_batch_size % 2 != 0:
            raise ValueError(
                "CFG local-AR input batch must contain equal conditional/unconditional halves, "
                f"got {input_batch_size}"
            )
        if topk < 1 or topk > self.codebook_size + len(SpecialAudioToken):
            raise ValueError(f"topk must be in [1, {self.codebook_size + len(SpecialAudioToken)}], got {topk}")
        if not np.isfinite(temperature):
            raise ValueError(f"temperature must be finite, got {temperature}")
        if not np.isfinite(cfg_scale):
            raise ValueError(f"cfg_scale must be finite, got {cfg_scale}")
        device_index = device.index
        if device_index is None:
            device_index = torch.cuda.current_device()
        return _LocalARCudaGraphKey(
    ...
    def _get_or_create_autoregressive_runtime(
        self,
        key: _LocalARCudaGraphKey,
        *,
        validate_cached: bool,
    ) -> _LocalARCudaGraphRuntime:
        with self._ar_cuda_graph_lifecycle:
            while self._ar_cuda_graph_invalidation_active:
                self._ar_cuda_graph_lifecycle.wait()
            runtime = self._ar_cuda_graph_runtimes.get(key)
            if runtime is None:
                runtime = _LocalARCudaGraphRuntime(self, key)
                self._ar_cuda_graph_runtimes[key] = runtime
            elif validate_cached:
                runtime.validate_parameter_invariants()
            return runtime

    def _acquire_autoregressive_runtime(self, key: _LocalARCudaGraphKey) -> _LocalARCudaGraphRuntime:
        with self._ar_cuda_graph_lifecycle:
            while self._ar_cuda_graph_invalidation_active:
                self._ar_cuda_graph_lifecycle.wait()
            runtime = self._ar_cuda_graph_runtimes.get(key)
            if runtime is None:
                runtime = _LocalARCudaGraphRuntime(self, key)
                self._ar_cuda_graph_runtimes[key] = runtime
            self._ar_cuda_graph_active_runs += 1
            return runtime

    def _release_autoregressive_runtime(self) -> None:
        with self._ar_cuda_graph_lifecycle:
            self._ar_cuda_graph_active_runs -= 1
            if self._ar_cuda_graph_active_runs == 0:
                self._ar_cuda_graph_lifecycle.notify_all()

    def invalidate_autoregressive_graphs(self) -> None:
        """Discard captured local-AR graphs before parameters are changed.

        Parent models must call this method before replacing or moving any
        module parameters referenced by this helper. Invalidation blocks new
        capture and replay, waits for active callers, and then synchronizes each
        private execution stream before releasing its graph-owned buffers.
        """

        with self._ar_cuda_graph_lifecycle:
            while self._ar_cuda_graph_invalidation_active:
                self._ar_cuda_graph_lifecycle.wait()
            self._ar_cuda_graph_invalidation_active = True
            while self._ar_cuda_graph_active_runs:
                self._ar_cuda_graph_lifecycle.wait()
            runtimes = tuple(self._ar_cuda_graph_runtimes.values())
            self._ar_cuda_graph_runtimes.clear()
    ...
    def warmup_autoregressive(
        ...
        *,
        ...
        """Compile and capture a local-AR graph before serving requests.

        ``actual_batch_size`` excludes the unconditional CFG half. This method
        performs the one-time device-wide synchronization needed by CUDA Graph
        capture. Request-time replay never performs a device-wide
        synchronization.
        """

        self._require_autoregressive_inference_mode()
        if actual_batch_size < 1:
            raise ValueError(f"actual_batch_size must be positive, got {actual_batch_size}")
        input_batch_size = actual_batch_size * (2 if use_cfg else 1)
        key = self._build_autoregressive_graph_key(
            device=device,
            dtype=dtype,
            input_batch_size=input_batch_size,
            input_dim=input_dim,
        ...
        self._get_or_create_autoregressive_runtime(key, validate_cached=True)

    def create_autoregressive_direct_capture_runtime(
        self,
        *,
        actual_batch_size: int,
        input_dim: int,
        ...
        """Create uncached state owned by a larger, directly captured graph."""
        ...
        return LocalARDirectCaptureRuntime(self, key)
    ...
    def sample_autoregressive(
        ...
        temperature: float = 0.7,
        topk: int = 80,
        unfinished_items: dict[int, bool] | None = None,
        finished_items: dict[int, bool] | None = None,
        use_cfg: bool = False,
        cfg_scale: float = 1.0,
        ...
        Args:
            ...
            output_codes: Caller-owned ``(B, C, S)`` int64 output buffer.
            ...
            random_state: Session-owned stochastic state. Required when
        ...
        """
        self._require_autoregressive_inference_mode()
        if dec_output.device.type != "cuda":
            raise ValueError(f"The local-AR inference runtime requires CUDA, received {dec_output.device}")
        if dec_output.ndim != 2:
            raise ValueError(f"dec_output must have shape (B, E), received {tuple(dec_output.shape)}")
        unfinished_items = {} if unfinished_items is None else unfinished_items
        finished_items = {} if finished_items is None else finished_items
        key = self._build_autoregressive_graph_key(
            device=dec_output.device,
            dtype=dec_output.dtype,
            input_batch_size=dec_output.size(0),
            input_dim=dec_output.size(1),
            temperature=temperature,
            topk=topk,
            use_cfg=use_cfg,
            cfg_scale=cfg_scale,
        )
        runtime = self._acquire_autoregressive_runtime(key)
        try:
            return runtime.run(
            ...
            self._release_autoregressive_runtime()
