...
class MagpieInferenceRunner(BaseInferenceRunner):
    ...
    def _warmup_local_ar_graphs(self, dataset_size: int) -> None:
        """Capture every local-AR graph shape used by this dataloader."""
        ...
        if (
            dataset_size == 0
            or not self.config.use_local_transformer
            or self.model.local_transformer_type != LocalTransformerType.AR
        ):
            return

        batch_sizes = {min(self.config.batch_size, dataset_size)}
        remainder = dataset_size % self.config.batch_size
        if remainder:
            batch_sizes.add(remainder)
        for batch_size in sorted(batch_sizes):
            self.model.warmup_local_ar_graph(batch_size=batch_size, use_cfg=self.config.use_cfg)
    ...
    def _run_unified_inference(
        ...
        self._warmup_local_ar_graphs(dataset_size=len(dataset))
